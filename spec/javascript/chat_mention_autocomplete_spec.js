...
// ---- normalizeInitialMentions(編集フォームのHydration結果を初期Stateへ正規化) ----
// (対象: Chat::MentionHydratorが埋め込むdata-initial-mentionsをinitMentionAutocompleteの
//  初期状態にする経路。DOM(textarea)自体は絡まないため純粋関数として検証できる)
test("初期mentionsが未指定なら空配列になる", function () {
  assert.deepStrictEqual(fns.normalizeInitialMentions(undefined), []);
  assert.deepStrictEqual(fns.normalizeInitialMentions(null), []);
});

test("配列でない値が渡されても例外にならず空配列になる", function () {
  assert.deepStrictEqual(fns.normalizeInitialMentions("not-an-array"), []);
  assert.deepStrictEqual(fns.normalizeInitialMentions({}), []);
});

test("Hydration結果の形をそのままStateへ反映する", function () {
  var initial = [{ customerId: 123, username: "tomoki", start: 0, end: 7 }];
  var result = fns.normalizeInitialMentions(initial);
  assert.deepStrictEqual(result, initial);
});

test("呼び出し元の配列・要素オブジェクトを直接変更しない(浅いコピー)", function () {
  var initial = [{ customerId: 123, username: "tomoki", start: 0, end: 7 }];
  var result = fns.normalizeInitialMentions(initial);
  result[0].start = 999;
  result.push({ customerId: 1, username: "x", start: 0, end: 1 });
  assert.strictEqual(initial[0].start, 0);
  assert.strictEqual(initial.length, 1);
});

test("start/endが数値でない不正な要素は除外される(壊れたJSON由来の防御)", function () {
  var result = fns.normalizeInitialMentions([
    { customerId: 1, username: "a", start: 0, end: 2 },
    { customerId: 2, username: "b" },
    null
  ]);
  assert.strictEqual(result.length, 1);
  assert.strictEqual(result[0].customerId, 1);
});

// ---- Hydration済みState → buildSubmissionContent の往復(内部記法が保存で失われないこと) ----
test("Hydrationされた表示テキストと初期mentionsをそのまま保存すると内部記法へ戻る", function () {
  // Chat::MentionHydrator.call(\"こんにちは [@tomoki](customer:123) さん\") 相当の出力を模す
  var visibleValue = "こんにちは @tomoki さん";
  var mentions = fns.normalizeInitialMentions([{ customerId: 123, username: "tomoki", start: 6, end: 13 }]);
  var content = fns.buildSubmissionContent(visibleValue, mentions);
  assert.strictEqual(content, "こんにちは [@tomoki](customer:123) さん");
});

test("Hydration済みStateのままメンション文字列を削除すると内部記法へ戻らず通常テキストになる", function () {
  var before = "こんにちは @tomoki さん";
  var mentions = fns.normalizeInitialMentions([{ customerId: 123, username: "tomoki", start: 6, end: 13 }]);
  var after = "こんにちは  さん"; // "@tomoki"を削除
  var adjusted = fns.diffAndAdjustMentions(before, after, mentions);
  assert.strictEqual(adjusted.length, 0);
  assert.strictEqual(fns.buildSubmissionContent(after, adjusted), "こんにちは  さん");
});

test("Hydration済みStateのままメンションより前へ文字を追加するとoffsetが追従し内部記法が維持される", function () {
  var before = "@tomoki さん";
  var mentions = fns.normalizeInitialMentions([{ customerId: 123, username: "tomoki", start: 0, end: 7 }]);
  var after = "こんにちは、" + before;
  var adjusted = fns.diffAndAdjustMentions(before, after, mentions);
  assert.strictEqual(adjusted.length, 1);
  assert.strictEqual(after.slice(adjusted[0].start, adjusted[0].end), "@tomoki");
  assert.strictEqual(fns.buildSubmissionContent(after, adjusted), "こんにちは、[@tomoki](customer:123) さん");
});
