...
RSpec.describe SongMasters::BackfillSongs do
  ...
  describe "dry_run: true" do
    it "DBを一切変更せず、実行予定だけを返すこと" do
      canonical = SongMasters::Resolver.call(song_name: "予定曲", artist_name: "予定P")
      wrong = FactoryBot.create(
        :song_master,
        song_name: "予定曲（予定P）",
        normalize: false,
        normalized_song_name: SongMasters::Resolver.normalize("予定曲（予定P）"),
        normalized_artist_name: ""
      )
      song = FactoryBot.create(:song, event: event, song_name: "予定曲（予定P）", artist_name: nil)
