class Public::CommunitiesController < ApplicationController
  ...
  def show
    ...
    @community_customers = params[:part_id].present? ? Kaminari.paginate_array(Part.find(params[:part_id]).customers.filter {|customer| customer.community_customers.where(community_id: @community.id).present? } ).page(params[:page]).per(8) : @community.customers.page(params[:page]).per(8)
  end
